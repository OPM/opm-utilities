...
HNET=DZ.*NTG;					# Calculate net cell height
...
WPAR_3D(C_index) = HNET(BOX);			# Put active cell data into 3-D array
ACT_3D(C_index) = 1;				# Define active cells in 3-D array

PAR2D_avg = sum(PAR_3D, 3)./sum(WPAR_3D, 3);	# Calulate 2-D array with average k-layers, DIM=3  
PAR3D_avg = repmat(PAR2D_avg, [1 1 DIM(3)]); 	# Copy 2-D array to 3-D array in K-direction
PAR_AVG = PERMX*NaN;
PAR_AVG(BOX) = PAR3D_avg(C_index);		# Copy active cell in 3-D array to 1-D array

riSetActiveCellProperty(PAR_AVG, "KX_avg_by_netH");	# Export to ri with proper name
