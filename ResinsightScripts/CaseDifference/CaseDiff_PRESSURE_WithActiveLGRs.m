...
# Calculate the difference in Dynamic properties between two spesified cases and write result back to case #2
...
PropNames = {"PRESSURE"};   	# Specify one or multiple Result properties. e.g. {"SOIL" "PRESSURE"}
#----------------------------------------------------------------------
...
for P = PropNames		
	...
	if all(size(P1)==size(P2))					# Check match of i,j,k,ts dimension 
		...
		riSetActiveCellProperty(DIFF_P, Case(CaseNo(2)).CaseId, DiffName);	# Write diff to case2
