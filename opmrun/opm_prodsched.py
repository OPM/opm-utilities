...
#
from opm_common import file_lstrip , opm_popup, opm_header_file, opm_view
...
def prodsched_data(file_in, file_inc, options, window, opmsys):
    ...
    out the schedule include file. In addition, a debug file (*.dbg) is also written out to verify the the results of
    the processing.
    ...
    options : list
        options[0] : Production generation option, set to daily, monthly, or onstream for first value in list
        options[1] : THP and BHP conversion from gauge to absolute conversion factor etc.
        options[2] : Control mode (ORAT,GRAT etc.)
    ...
    wat_lst = ['wat', 'water rate', 'wrat', 'WAT', 'WATER RATE', 'WRAT', 'Prd_dly.Water']
    ...
    if err_count > 0:
        ...
        return (err_count)
    #
    # Drop Unnecessary Columns, Replace Default Pressure Values with Zero, and Start Processing Data
    #
    ...
    for column_name in df:
        if column_name in list:
            sg.cprint('    ' + text + ' column found as ' + column_name + ' in Column ' +
                      str(df.columns.get_loc(column_name)))
            if column_name != name:
...
def prodsched_main(opmoptn, opmsys):
    ...
    pressure = ('No Conversion', 'atma to atmg', 'atmg to atma', 'barsg to barsa', 'barsa to barsg', 'psig to psig',
                'psig to psia')
    factors  = (0              ,   1.000       , 1.000         , 1.01325         , -1.01325        , 14.65,
               -14.65)
    ...
    layout1 = [
        [sg.Text('OPM_PROD_SCHED Option to Generate the OPM Flow WCONHIST Keywords from Daily Production Data')],
        [sg.Text('Exported Daily Well Production Data File')],
        ...
        [sg.Text('Output Log')],
        [sg.Multiline(key=outlog, size=(137, 20), text_color='blue', autoscroll=True,
    ...
    while True:
        ...
        #
            ...
            error = prodsched_data(file_in, file_inc, options, window1, opmsys)
            if error !=0:
                continue
